...
async def lifespan(app):
    ...
    for key in models.keys():
        # Check if the model is a PyTorch model or a TensorFlow model
        if hasattr(models[key], "model"):
            if hasattr(models[key].model, "cpu"):
                models[key].model.cpu()  # PyTorch model cleanup
            else:
                # For TensorFlow models, no need for `.cpu()` call
                pass
        models[key] = None
