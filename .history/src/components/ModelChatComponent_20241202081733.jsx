...
const ModelChatComponent = ({ ModelId }) => {
    ...
    }, [messages]); // Add errorMessage to dependencies

    // Handle sending message
    const handleSendMessage = async () => {
        if (input.trim()) {
            try {
                dispatch(sendMessage(input)); // Dispatch the send message action
        ...
        <div className='chat-container bg-light-glass'>
            ...
            {messages.map((msg, index) => (
                    <motion.div
                        ...
                        transition={{ delay: index % 2 === 0 ? 0 : 1, duration: 0.5 }} // Opóźnienie 0 dla pierwszej w parze, 1 sekunda dla drugiej
                    >
                        {msg.text}
                    </motion.div>
                ))}
