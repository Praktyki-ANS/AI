...
import { sendMessage } from '../redux/chatSlice'; // Zakładam, że masz odpowiednią ścieżkę
...
const ModelChatComponent = ({ ModelId }) => {
  ...
  const { messages, isSending: reduxIsSending, error } = useSelector(state => state.chat); // Redux state
  ...
  const handleSendMessage = () => {
    if (input.trim() && !isSending) { // Prevent sending while generating echo
      dispatch(sendMessage(input)); // Dispatch message to Redux
      setMessages((prevMessages) => [...prevMessages, { text: input, isUser: true }]); // Update local state
      setInput('');
    }
  };
