...
import { navLinks, Chats } from '../constants';  // Import Chats array from your constants
    ...
    if (isChatDetailPage) {
      return Chats.map((chat) => (
        <li
          key={chat.id}
          className={`${
            active === chat.title ? 'text-white' : 'text-secondary'
          } hover:text-white text-[18px] font-medium cursor-pointer`}
          onClick={() => setActive(chat.title)}
        >
          <Link to={`/ANS-AI-DEMO/chat/${chat.id}`}>{chat.title}</Link>
        </li>
      ));
    }
