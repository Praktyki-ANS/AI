...
const ModelChatComponent = ({ ModelId }) => {
  ...
  // Handle the response (either echo or error)
  useEffect(() => {
    if (messages.length > 0 && messages[messages.length - 1].isUser) {
      ...
      setTimeout(() => {
        ...
        // Simulate response (either echo or error)
        const isError = Math.random() < 0.5; // Simulate error condition (50% chance for error)
        const responseText = isError ? "No response available" : `Echo: ${messages[messages.length - 1].text}`;

        dispatch(sendMessage({ text: responseText, isUser: false }));
      }, 2000); // Simulate a typing delay of 2 seconds
    }
  }, [messages, dispatch]);
