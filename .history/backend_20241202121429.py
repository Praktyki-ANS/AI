...
async def lifespan(app):
    ...
    for key in models.keys():
        # Check if the model has a 'model' attribute (i.e., it is a Hugging Face pipeline)
        if hasattr(models[key], "model"):
            model = models[key].model
            if hasattr(model, "cpu"):  # Check if it's a PyTorch model
                model.cpu()  # Move the model to CPU to free GPU memory
            else:
                pass  # No cleanup required for TensorFlow models
        models[key] = None
